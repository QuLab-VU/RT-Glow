...
```{r Cell count & luminescence linear scale, eval=FALSE, fig.height=3, fig.width=7, include=FALSE}
par(mfrow=c(2,5))
...
```{r cell count to lum, fig.height=9, fig.width=6}
par(mfrow=c(3,2))
...
```{r cell count to lum v2, fig.height=2, fig.width=8}
par(mfrow=c(1,3))
cl <- "H841"
dtp <- ctrls[[cl]]
# normalized log2 scale (population doublings)
invisible(do.call(plotGC, append(getGCargs(dtp, dat.col=c("time","RLU","uid")),
                                 list(main="H841, lum", leg=FALSE, ylim=c(-1,3)))))
lines(sumc[sumc$cell.line==cl,"time"], sumc[sumc$cell.line==cl,"popdubs"], lwd=3)

# linear scale (no normalization)
plot(RLU ~ time, data=dtp, type="n")
invisible(sapply(unique(dtp$well), function(w) 
    lines(dtp[dtp$well==w,"time"],dtp[dtp$well==w,"RLU"], col="blue")))

# linear scale, proportional to time 0
plot(0:200, 0:200, type="n", ylim=c(0,10))
invisible(sapply(unique(dtp$well), function(w) 
    lines(dtp[dtp$well==w,"time"],dtp[dtp$well==w,"RLU"]/dtp[dtp$well==w,"RLU"][1], col="blue")))
```

```{r}
# log scale (no normalization)
plot(log2(RLU) ~ time, data=dtp, type="n")
invisible(sapply(unique(dtp$well), function(w) 
    lines(dtp[dtp$well==w,"time"],log2(dtp[dtp$well==w,"RLU"]), col="blue")))
lines(sumc[sumc$cell.line==cl,"time"], sumc[sumc$cell.line==cl,"cell.count"], lwd=3)
```
...
# conform all data is same as Thunor
all(lcc$Cell_Count==d$Cell_Count)
```
