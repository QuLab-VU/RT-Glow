...
```{r Time matching}
# Cell count file path =  ccfp; from task arguments sent to py-seg image processing pipeline
ccfp <- read.csv(file.path(TOPDIR,'TaskArgs_20201214.csv'))$nuc_im_path
# Find times for each unique plate
ccfp <- gsub("/mnt/monica/quaranta2/Cytation/2020-12-14/images/","",ccfp)
ccfp <- unique(sapply(strsplit(ccfp, "/"), "[[", 1))

# get plate IDs from number after "Experiment"; should match Momentum log
pid <- as.integer(sapply(strsplit(ccfp,"Experiment"), "[[", 2))
names(pid) <- getDateTime(ccfp)
