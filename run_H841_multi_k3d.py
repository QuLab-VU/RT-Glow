...
rate_mask = np.array([True, False, False, True, True, False, True, True, True, True, True, False, False, False, False, False])

original_values = np.array([p.value for p in model.parameters])
k5 = original_values[1]
k6 = original_values[2]

k5d_idx = np.where(param_names[rate_mask] == 'K5d')[0]
k6d_idx = np.where(param_names[rate_mask] == 'K6d')[0]
...
def costfunction(parameter):
    ...
    # k5d(T), k6d(T)
    if k5 < parameter[k5d_idx]:
        return 1e8
    # k6 needs to keep smaller than k6d
    if k6 > parameter[k6d_idx]:
        return 1e8
    # error_static = 1*obj_static(parameter)
    # print('error static: ', error_static)

    error0 = obj_dynamic(parameter, 300, 0.15e-9*N_A*80e-6, exp_data['cleaned_0.15nM_mean'], tol)
